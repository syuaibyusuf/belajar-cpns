...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 ml-64 p-8">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 max-w-4xl">
            <h1 class="text-2xl font-bold mb-6">✏️ Edit Soal</h1>

            <form method="POST" action="{{ route('admin.soal.update', $question->id) }}" id="soalForm">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="category" id="category" class="w-full px-4 py-2 border rounded-lg" required onchange="toggleTKPMode()">
                        ...
                        <option value="{{ $key }}" {{ $question->category == $key ? 'selected' : '' }}>{{ $cat['name'] }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="question_text" id="question_text" rows="4" class="w-full px-4 py-2 border rounded-lg" required>{{ $question->question_text }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="hidden" name="question_image" id="question_image_base64" value="{{ $question->question_image }}">
                    <div id="question_image_preview" class="mt-2 {{ $question->question_image ? '' : 'hidden' }}">
                        <img id="question_preview_img" src="{{ $question->question_image }}" class="max-w-full h-32 object-cover rounded-lg border">
                ...
                <div class="grid md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="border rounded-lg p-3">
                        ...
                        <input type="text" name="option_{{ $opt }}" value="{{ $question->{'option_' . $opt} }}" class="w-full px-3 py-2 border rounded-lg" required>
                        ...
                        <input type="hidden" name="image_{{ $opt }}" id="image_{{ $opt }}_base64" value="{{ $question->{'image_' . $opt} }}">
                        <div id="image_{{ $opt }}_preview" class="mt-1 {{ $question->{'image_' . $opt} ? '' : 'hidden' }}">
                            <img id="image_{{ $opt }}_preview_img" src="{{ $question->{'image_' . $opt} }}" class="max-w-full h-20 object-cover rounded-lg border">
                        </div>
                        
                        <div class="tkp-score mt-2 {{ $question->category == 'tkp' ? '' : 'hidden' }}">
                            <label class="block text-gray-700 mb-1 text-sm">Nilai (1-5)</label>
                            <input type="number" name="score_{{ $opt }}" min="1" max="5" value="{{ $question->{'score_' . $opt} ?? 0 }}" class="w-24 px-3 py-1 border rounded-lg">
                        </div>
                    </div>
                    @endforeach
                </div>

                <div id="twk_tiu_section" class="mb-4 {{ $question->category == 'tkp' ? 'hidden' : '' }}">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">Pembahasan</label>
                    <textarea name="explanation" rows="3" class="w-full px-4 py-2 border rounded-lg">{{ $question->explanation }}</textarea>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        <label class="block text-gray-700 mb-2">Poin</label>
                        <input type="number" name="points" value="{{ $question->points }}" class="w-32 px-4 py-2 border rounded-lg">
                    </div>
                </div>
...
<script>
    function toggleTKPMode() {
        ...
        if (category === 'tkp') {
            twkTiuSection.classList.add('hidden');
            tkpScores.forEach(el => el.classList.remove('hidden'));
        } else {
            twkTiuSection.classList.remove('hidden');
